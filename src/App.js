

import MovieList from "./MovieList";


function App() {

  return (
    <div className="flex flex-col items-center h-scree">
      <div className="mb-32 bg-[#39424E] w-full p-10 flex items-center justify-center">
        <img src="data:image/png;base64," alt="logo" className="w-20 h-20" />
        <span className="text-[#22C55E] text-2xl ml-2">MovieList</span>
      </div>
      <header className="flex w-full h- items-center justify-center">
        <MovieList />
      </header>
    </div>
  );
}

export default App;
